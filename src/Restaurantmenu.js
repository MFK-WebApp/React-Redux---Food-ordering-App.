...
const RestaurantMenu = () => {
  ...
  async function getRestaurantInfo() {
    const data = await fetch(
      "https://www.swiggy.com/dapi/menu/pl?page-type=REGULAR_MENU&complete-menu=true&lat=27.9079413&lng=78.07660360000001&restaurantId=" + resId + "&catalog_qa=undefined&submitAction=ENTER"
    );
  ...
  <div className="menuCard bg-purple-600 text-white p-4 rounded-lg mb-8 w-full md:w-1/3 ">
    ...
    <img src={IMG_CDN_URL + restaurant.cloudinaryImageId} className="w-full mb-4" alt="Restaurant" />
  ...
  <div className="list-style ml-10 p-2 pl-5 bg-purple-300 rounded-lg w-3/4">
    ...
    <ul>
      {Object.values(resmenu?.data.cards[2].groupedCard.cardGroupMap.REGULAR.cards[1].card.card.itemCards.map(
          (itemCard) => (
            <li key={itemCard?.card?.info?.id} className="text-lg mb-2">{itemCard?.card?.info?.name}</li>
          )
        ))}
    </ul>
  </div>
